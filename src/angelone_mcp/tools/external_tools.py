...
def _fetch_google_news_rss(query: str, max_items: int = 5) -> List[Dict[str, str]]:
...
def _fetch_yahoo_news_rss(query: str, max_items: int = 5) -> List[Dict[str, str]]:
...
def register_external_tools(mcp: FastMCP) -> None:
    ...
    Tools:
      - scrape_stock_news_summaries(keywords: list[str]) -> dict[str, str]
        Returns a mapping of provider name to a single formatted string containing
        bullet-pointed news items (title and link) for each provider.
    """

    @mcp.tool(
        name="scrape_stock_news_summaries",
        description=(
            "Aggregate recent stock news snippets from multiple public providers "
            "for the given keywords. Providers include: google_news, yahoo_news, "
            "moneycontrol, economic_times_markets, business_standard_markets. "
    ...
    def scrape_stock_news_summaries(keywords: List[str]) -> JSONValue:
        ...
        Returns:
          dict[str, str]: Mapping of provider to text block with bullet-pointed
          headlines and links.
        """
        ...
        try:
            ...
            providers["google_news"] = _format_items_as_bullets(google_items) or "No results"
        except Exception as exc:  # noqa: BLE001 - surface provider-specific failure
            providers["google_news"] = f"Error: {exc}"

        try:
            yahoo_items = _fetch_yahoo_news_rss(query)
            providers["yahoo_news"] = _format_items_as_bullets(yahoo_items) or "No results"
        ...
        try:
            ...
            providers["moneycontrol"] = _format_items_as_bullets(mc_items) or "No results"
        except Exception as exc:  # noqa: BLE001
        ...
        try:
            ...
            providers["economic_times_markets"] = _format_items_as_bullets(et_items) or "No results"
        except Exception as exc:  # noqa: BLE001
        ...
        try:
            ...
            providers["business_standard_markets"] = _format_items_as_bullets(bs_items) or "No results"
        except Exception as exc:  # noqa: BLE001
