...
from SmartApi.version import __title__, __version__
...
class SmartConnect(object):
    ...
    try:
        ...
        logger.error(
            f"Exception while retriving IP Address,using local host IP address: {e}"
        )
    finally:
    ...
    def _request(self, route, method, parameters=None):
        ...
        if "json" in headers["Content-type"]:
            ...
            if data.get("status", False) is False:
                logger.error(
                    f"Error occurred while making a {method} request to {url}. Error: {data['message']}. URL: {url}, Headers: {self.requestHeaders()}, Request: {params}, Response: {data}"
                )
            return data
    ...
    def make_authenticated_get_request(self, url, access_token):
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Error in make_authenticated_get_request: {response.status_code}"
            )
            return None
